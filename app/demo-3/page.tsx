...
import {
  FreelanceLoanProfileActionKey,
  FreelanceLoanProfileWidget,
} from "@/src/chat-ui/components/widgets/FreelanceLoanProfileWidget";
import {
  CarInstallmentPaidActionKey,
  CarInstallmentPaidWidget,
} from "@/src/chat-ui/components/widgets/CarInstallmentPaidWidget";
import {
  FreelanceLoanSummaryActionKey,
  FreelanceLoanSummaryWidget,
} from "@/src/chat-ui/components/widgets/FreelanceLoanSummaryWidget";
import {
  FreelanceLoanSubmittedActionKey,
  FreelanceLoanSubmittedWidget,
} from "@/src/chat-ui/components/widgets/FreelanceLoanSubmittedWidget";
import { FreelanceAssessmentResultWidget } from "@/src/chat-ui/components/widgets/FreelanceAssessmentResultWidget";
...
type ChatMessage = {
  ...
  widget?:
    | "bill_table"
    | "insufficient_funds"
    | "freelance_loan_profile"
    | "freelance_assessment_result"
    | "freelance_loan_summary"
    | "freelance_loan_submitted"
    | "car_payment_success";
};
...
export default function DemoTwoPage() {
  ...
  const [pinIntent, setPinIntent] = React.useState<"freelance_confirm" | "pay_car" | null>(null);
  ...
  const startDocsAssessmentFlow = React.useCallback(() => {
    clearPendingReplyTimer();
  ...
  const handlePdfSelected = React.useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!isPdf) {
        appendMessage("assistant", "กรุณาอัปโหลดไฟล์ PDF เท่านั้นครับ");
        event.target.value = "";
        return;
      }
      appendMessage("user", `แนบไฟล์: ${file.name}`);
      event.target.value = "";
      startDocsAssessmentFlow();
    },
    [appendMessage, startDocsAssessmentFlow],
  );
  ...
  const handleFreelanceLoanProfileAction = React.useCallback(
    (action: FreelanceLoanProfileActionKey) => {
      if (isThinking || isStreamingReply) return;
      if (action === "has_docs") {
        appendMessage("user", "มี ส่งเอกสารเลย");
        openPdfPicker();
        return;
      }
      appendMessage("user", "ไม่มีเอกสาร");
      scheduleAssistantReply("ไม่เป็นไรครับ ผมสามารถแนะนำทางเลือกอื่นให้คุณเจนได้");
    },
  ...
  const handleFreelanceLoanSubmittedAction = React.useCallback(
    (action: FreelanceLoanSubmittedActionKey) => {
      if (isThinking || isStreamingReply) return;
      if (action === "pay_other") {
      ...
      if (action === "check_loan_status") {
        appendMessage("user", "ตรวจสอบสถานะสินเชื่อ");
        scheduleAssistantReply("สถานะล่าสุด: คำขอกำลังรอพิจารณา และจะแจ้งผลภายในวันที่ 5 มี.ค. 2569 ครับ");
        return;
      }
      appendMessage("user", "เสร็จแล้วค่ะ");
  ...
  const submitLoanPin = React.useCallback(() => {
    ...
    } else if (pinIntent === "freelance_confirm") {
      scheduleAssistantReply("", "freelance_loan_submitted");
    } else {
      scheduleAssistantReply(LOAN_APPROVED_MESSAGE);
    }
    setPinIntent(null);
  ...
  const handleSend = React.useCallback(() => {
    ...
    if (normalized === "มี" || normalized === "มีค่ะ" || normalized.includes("มีค่ะส่งเอกสารเลย")) {
      openPdfPicker();
      setInputText("");
      return;
    }
                    ...
                    >
                      {message.role === "assistant" ? (
                        <>
                          ...
                          {message.widget === "freelance_loan_profile" ? (
                            <FreelanceLoanProfileWidget
                              disabled={isThinking || isStreamingReply}
                              onAction={handleFreelanceLoanProfileAction}
                            />
                          ) : null}
                          {message.widget === "freelance_assessment_result" ? (
                            <FreelanceAssessmentResultWidget />
                          ) : null}
                          {message.widget === "freelance_loan_summary" ? (
                            <FreelanceLoanSummaryWidget
                              disabled={isThinking || isStreamingReply}
                              onAction={handleFreelanceLoanSummaryAction}
                            />
                          ) : null}
                          {message.widget === "freelance_loan_submitted" ? (
                            <FreelanceLoanSubmittedWidget
                              disabled={isThinking || isStreamingReply}
                              onAction={handleFreelanceLoanSubmittedAction}
                            />
                          ) : null}
